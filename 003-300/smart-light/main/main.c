...
void app_main(void) {
  ...
  while (1) {
    ...
    if (tcp_client_is_connected()) {
        remote_power_on = tcp_client_get_power_state_from_remote(&remote_commanded_power_this_session);
        if (remote_commanded_power_this_session && remote_power_on) {
            // 远程指令开灯，优先级最高
            tcp_client_apply_current_led_settings();
            vTaskDelay(pdMS_TO_TICKS(500)); // 保持远程状态的检查频率
            continue;
        }
        // 如果只是连接着，但没有明确的远程开灯指令，则允许自动逻辑执行，但远程关灯优先级最低
    }
    ...
    if (pir_detected()) {
      ...
      if (light < 200) { // 光强较暗
        // 如果远程没有明确关灯，则执行自动开灯
        if (!(tcp_client_is_connected() && remote_commanded_power_this_session && !remote_power_on)) {
